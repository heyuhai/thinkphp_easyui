...
<div class="contentBox">
    <table id="treeModule" title="模块列表" class="easyui-treegrid" style="width:98%;height:500px;" url="<?php echo U('Sysadmin/sysmoduleJsonData');?>" rownumbers="true" idField="id" treeField="title" toolbar="#tb">
        <thead>
            <tr>
                <th field="title" width="200" align="left">模块名称</th>
                <th field="module" width="200" align="center">module</th>
                <th field="action" width="200" align="center">action</th>
                <th field="param" width="200" align="center">param</th>
            ...
            <div data-options="region:'center'" style="padding:10px;">
                <form id="ff" method="post">
                    <table cellpadding="5">
                        <tr>
                            ...
                            <td>
                                <select style="width:140px;" class="easyui-combobox" name="pid">
                                    ...
                                    <?php if(is_array($pList)): $i = 0; $__LIST__ = $pList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></option>
                                        <?php foreach($vo['children'] as $val): ?>
                        ...
                        <tr>
                            <td>module:</td>
                            <td><input class="easyui-textbox" type="text" name="module" style="width:230px;" data-options="required:true"></input></td>
                        </tr>
                        <tr>
                            <td>action:</td>
                            <td><input class="easyui-textbox" type="text" name="action" style="width:230px;" data-options="required:true"></input></td>
                        </tr>
                        <tr>
                            <td>param:</td>
                            <td><input class="easyui-textbox" type="text" name="param" style="width:230px;"></input></td>
                        </tr>
                        <tr>
                            <td>排序:</td>
    ...
    <script type="text/javascript">
        ...
        function addrow()
        {
            ...
            rows = eval('(' + addRows + ')');
            url = "<?php echo U('Sysadmin/sysmoduleModify','',''); ?>";
            $('#ff').form('load', rows);
            $('#w').window('open');
        }
        function editrow()
        {
            ...
            if (rows) {
                url = "<?php echo U('Sysadmin/sysmoduleModify','',''); ?>?id="+rows.id;
                $('#ff').form('load', rows);
                $('#w').window('open');
            }
            else {
        ...
        function delrow()
        {
            ...
            if (rows) {
                $.messager.confirm('提示',confirmMsg,function(r){
                    if (r){
                        $.ajax({
                            url: "<?php echo U('Sysadmin/sysmoduleDel','',''); ?>?id="+rows.id,
                            dataType: "json",
